...
public:
    ...
    static void CompositeInitializer(GAGenome& genome) {
        CompositeGenome& compositeGenome = static_cast<CompositeGenome&>(genome);
        compositeGenome.xPositions->initialize();
        compositeGenome.yPositions->initialize();
        compositeGenome.isUsed->initialize();
    ...
    static int CompositeMutator(GAGenome& genome, float mutationProbability) {
        CompositeGenome& compositeGenome = static_cast<CompositeGenome&>(genome);
        int numberOfMutations = compositeGenome.xPositions->mutate(mutationProbability) 
            + compositeGenome.yPositions->mutate(mutationProbability) 
            + compositeGenome.isUsed->mutate(mutationProbability)
            + compositeGenome.isRotated->mutate(mutationProbability);
        if (numberOfMutations) compositeGenome._evaluated = gaFalse;
